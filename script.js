...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // =====================
    // RANCH MAIN — YouTube Walkthrough (autoplay + text overlays)
    // =====================
    const walkthroughSection = document.getElementById('walkthroughSection');

    // YouTube IFrame API — initialize player
    let ytPlayer;
    window.onYouTubeIframeAPIReady = function () {
        ytPlayer = new YT.Player('walkthroughYT', {
            videoId: 'NL72YvDKvKg',
            playerVars: {
                autoplay: 1,
                mute: 1,
                loop: 1,
                playlist: 'NL72YvDKvKg',
                controls: 0,
                rel: 0,
                modestbranding: 1,
                playsinline: 1
    ...
    // Play/pause YouTube based on scroll visibility
    ScrollTrigger.create({
        trigger: walkthroughSection,
        start: 'top bottom',
        end: 'bottom top',
        onEnter: () => { if (ytPlayer && ytPlayer.playVideo) ytPlayer.playVideo(); },
        onLeave: () => { if (ytPlayer && ytPlayer.pauseVideo) ytPlayer.pauseVideo(); },
        onEnterBack: () => { if (ytPlayer && ytPlayer.playVideo) ytPlayer.playVideo(); },
        onLeaveBack: () => { if (ytPlayer && ytPlayer.pauseVideo) ytPlayer.pauseVideo(); }
    });

    // Text overlays (scroll-driven, independent of video)
    [
